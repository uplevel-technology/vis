 ...
 *            maxStackHeight for stacking
 */
exports.stack = function(items, margin, force, maxStackHeight) {
  if (force) {
  ...
  for (var i = 0; i < items.length; i++) {  // eslint-disable-line no-redeclare
    ...
    if (item.stack && item.top === null) {
      ...
      if (maxStackHeight < Infinity) {
        // reset all points first
        item.dom.point.style.display = 'block';
      }
      ...
      do {
        ...
        for (var j = 0, jj = items.length; j < jj; j++) {
          var other = items[j];
          if (other.top <= maxStackHeight && other.top !== null && other !== item && other.stack && exports.collision(item, other, margin.item, other.options.rtl)) {
            collidingItem = other;
            break;
          }
        }
        ...
        if (collidingItem != null) {
          ...
          if (item.top > maxStackHeight) {
            item.dom.point.style.display = 'none';
          }
        }
      } while (collidingItem);
    }
